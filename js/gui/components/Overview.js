...
Overview.prototype = {
	 ...
	 */
	update: function( characterFacade ) {
		var hrd, span, gear, gearContainer, itmParent, i, j;
		var statHandler = new Handler( function( e ) {
			if( e.is('stat_tooltip_show') ) {
				Tooltip.showStat( characterFacade.getStatTooltip(e.get('group'), e.get('index')), e.get('node') );
			}
		...
		DOM.createAt( hrd, 'span', { 'class': 'ov_name', 'text': characterFacade.name ? characterFacade.name + ',' : ''});
		DOM.createAt( hrd, 'span', { 'class': 'ov_level', 'text': "Level " + characterFacade.level});
		
		if( characterFacade.getCharacterRaceId() > 0 ) {
			
			DOM.createAt( hrd, 'span', { 'class': 'ov_character_race', 'text': locale['CharacterRace'][characterFacade.getCharacterRaceId()]});
			
			if( characterFacade.getCharacterClassId() > 0 ) {

				span = DOM.createAt( hrd, 'span', { 'class': 'ov_character_class', 'text': locale['CharacterClass'][characterFacade.getCharacterClassId()]});
				
				DOM.addClass(span,'character_class_'+characterFacade.getCharacterClassId());
				
				if( characterFacade.getCharacterClassId() == PRIEST ) {
					DOM.addClass(span,'character_class_5_white_bg_fix');
				}
				else if( characterFacade.getCharacterClassId() == ROGUE ) {
		...
		DOM.createAt( gearContainer, 'div', {'class': 'ov_gear_title', 'text': 'Gear'});
		gear = DOM.createAt( gearContainer, 'div', {'class': 'ov_items_p'});
		
		for ( i=0; i<characterFacade.items.length; i++ ) {
			
			if(  characterFacade.items[i] == null ) {
				continue;
			} 

			DOM.createAt( gear, 'span', {'class': 'ov_slot_name', 'text': locale['Slots'][i]} );
			itmParent = DOM.createAt(gear, 'div', { 'class': 'ov_item_p'});
		...
		if( g_settings.profileId > 0 ) {
			...
			DOM.createAt( this.commentsParent, 'div', {'class': 'ov_comments_h', 'text': 'Comments:'});
		...
		this.__updateStats(characterFacade.getStats(), characterFacade.getCharacterClassId(), characterFacade.getShapeform());
	 ...
	 */
	__addItem: function( node, itm ) {
		var a, gemParent, gemsParent;
		if( itm ) {
			DOM.createAt( node, 'img', {'src': 'images/icons/small/'+itm.icon+'.png', 'class': 'ov_item_icon'});
			a = DOM.createAt( node, 'a', {'href': 'javascript:', 'class': 'ov_item_link item_quality_' + itm.quality, 'text': itm.name});
			ChardevHTML.addTooltip( a, itm.getTooltip() );
			
			gemsParent = DOM.createAt( node, 'div', {'class':'ov_gems_p'});
			for( var i=0; i<3; i++ ) {
